#!/usr/bin/env python3
"""
Medical Appointments API - Integrated with GCP Auto-scaling Infrastructure
Enhanced version of the Medical Appointments API for Cloud Run deployment
"""
import os
import json
import subprocess
...
if not DATABASE_URI:
    # Try GCP SQL if available
    DB_HOST = os.environ.get('DB_HOST', 'localhost')
    DB_USER = os.environ.get('DB_USER', 'root')
    DB_PASSWORD = os.environ.get('DB_PASSWORD', '')
    DB_NAME = os.environ.get('DB_NAME', 'medical_appointments')
    
    if all([DB_HOST, DB_USER, DB_NAME]) and DB_HOST != 'localhost':
        DATABASE_URI = f"mysql+pymysql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}/{DB_NAME}"
    else:
        # Fallback to SQLite for local development
        DATABASE_URI = 'sqlite:///medical_appointments.db'
...
try:
    ...
    gcp_available = True
    logger.info("GCP services connected successfully")
except Exception as e:
    logger.warning(f"GCP services not available: {e}")
    firestore_db = None
    storage_client = None
    gcp_available = False

# App configuration for auto-scaling
APP_CONFIG = {
